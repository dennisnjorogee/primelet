...
const Login = () => {
  const { register_user, login_user, forgot_password, loading } =
    useAppContext();
  ...
  const handleSubmit = async () => {
    ...
    if (view === "reset") {
      const result = await forgot_password({
        emailAddress: formData.emailAddress,
      });
      if (result) {
        setLocalError("");
    ...
    <div className="flex flex-col p-6 items-center justify-center min-h-screen">
      <div className="flex flex-col items-center gap-6 m-2 p-6 rounded-lg bg-gray-100 shadow-2xl w-full max-w-sm">
        ...
        <div className="flex flex-col items-center gap-4 w-full">
          ...
          {localError && (
            <p className="text-red-500 text-sm italic font-semibold bg-red-300/20 py-4 px-5 text-center">
              {localError}
            </p>
          )}
        </div>
